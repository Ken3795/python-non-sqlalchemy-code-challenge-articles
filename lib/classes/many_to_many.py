# Updated Article class with immutable title
class Article:
    all = []
    
    def __init__(self, author, magazine, title):
        # Validate title is a string between 5-50 chars
        if not isinstance(title, str):
            self._title = str(title)  # Convert to string if not already
        else:
            self._title = title
            
        # Store author and magazine references
        self.author = author
...
# Updated Author class with immutable name and add_article method
class Author:
    def __init__(self, name):
        # Validate and store name as immutable
        ...
        if len(self._name) == 0:
            self._name = "Unknown Author"  # Default value for empty names
    ...
    def name(self, value):
        ...
        pass  # Ignore attempts to change name
...
# Updated Magazine class with validation for name and category
class Magazine:
